                                ...
                                <table class="table">
                                    <thead>
                                        <tr>
                                            ...
                                            <th scope="col">Allow to write</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        <c:forEach var="al" items="${authorList}">
                                            <tr>
                                                ...
                                                <td scope="row"><img src="${al.image}" alt="" style="width: 60px; height: 34px;"></td>
                                                <td>${al.name}</td>
                                                <td>${al.email}</td>
                                                <td>${al.status}</td>
                                                <td>
                                                    <select id="statusSelect_${al.id}" class="badge ${al.status? 'bg-success' : 'bg-danger'}" onchange="updateBadgeClass('statusSelect_${al.id}')">
                                                        <option value="allowed" ${al.status? 'selected' : ''}>Allowed</option>
                                                        <option value="denied" ${!al.status? 'selected' : ''}>Denied</option>
                                                    </select>
                                                </td>
                                            </tr>
                                        </c:forEach> 
...
<script>
                        ...
                        function categorizeRows() {
                            ...
                            rows.forEach(function (row) {
                                ...
                                var selectedValue = selectElement.value;
                                var alId = selectElement.id.split('_')[1]; // Extract the apa.id from the id

                                if (selectedValue === "allowed") {
                                    aArr.push(parseInt(alId));
                                } else if (selectedValue === "denied") {
                                    dArr.push(parseInt(alId));
                                }
                            });

                            var servletUrl = "/Assigment/changeAuthorStatus"; // Replace with the actual URL of your servlet
                            ...
                            input1.type = "hidden";
                            input1.name = "aArr";
                            input1.value = aArr.join(",");
                            form.appendChild(input1);

                            var input2 = document.createElement("input");
                            input2.type = "hidden";
                            input2.name = "dArr";
                            input2.value = dArr.join(",");
