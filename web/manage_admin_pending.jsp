<%-- 
    Document   : manage_admin_pending
    Created on : Aug 24, 2023, 5:17:50 PM
    Author     : MSI
--%>
                        ...
                        <div class="card">
                            <div class="card-body">
                                ...
                                <table class="table">
                                    ...
                                    <tbody>
                                        <c:forEach var="apa" items="${allPendingArticles}">
                                            <tr>
                                                <th scope="row">${apa.id}</th>
                                                    <c:forEach var="b" items="${authorList}">
                                                        <c:if test="${apa.author_id eq b.id}">
                                                        <td>${b.name}</td>
                                                    </c:if>
                                                </c:forEach>
                                                <td>${apa.title}</td>
                                                ...
                                                <td>${apa.date}</td>
                                                <td>
                                                    <select id="statusSelect_${apa.id}" class="badge bg-warning" onchange="updateBadgeClass('statusSelect_${apa.id}')">
...
<script>
                        ...
                        function categorizeRows() {
                            ...
                            rows.forEach(function (row) {
                                ...
                                var selectedValue = selectElement.value;
                                var apaId = selectElement.id.split('_')[1]; // Extract the apa.id from the id

                                if (selectedValue === "pending") {
                                    pArr.push(parseInt(apaId));
                                } else if (selectedValue === "approved") {
                                    aArr.push(parseInt(apaId));
                                } else if (selectedValue === "rejected") {
                                    rArr.push(parseInt(apaId));
                                }
                            });
